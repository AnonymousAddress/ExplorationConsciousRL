    ...
    with U.single_threaded_session() as sess:
        ...
        for epoch in range(nb_epochs):
            for cycle in range(nb_epoch_cycles):
                ...
                #         eval_action, eval_q, _ = agent.pi(eval_obs, apply_noise=False, compute_Q=True)
                ...
                if eval_env is not None:
                    eval_episode_reward = 0.
                    while True:
                        eval_action, eval_q, _, _ = agent.pi(eval_obs, apply_noise=False, compute_Q=True)
                        eval_obs, eval_r, eval_done, eval_info = eval_env.step(max_action * eval_action)  # scale for execution in env (as far as DDPG is concerned, every action is in [-1, 1])
                        if render_eval:
                            eval_env.render()
                        eval_episode_reward += eval_r
                        ...
                        if eval_done:
                            eval_obs = eval_env.reset()
                            epoch_episode_eval_rewards.append(eval_episode_reward)
                            eval_episode_rewards_history.append(eval_episode_reward)
                            eval_episode_reward = 0.
                            break
            ...
            # combined_stats['steps'] = t
            # Evaluation statistics.
            if eval_env is not None:
                combined_stats['eval/return'] = np.mean(epoch_episode_eval_rewards)
                combined_stats['eval/return_history'] = np.mean(eval_episode_rewards_history)
                combined_stats['eval/Q'] = np.mean(eval_qs)
                combined_stats['eval/episodes'] = len(epoch_episode_eval_rewards)
            def as_scalar(x):
