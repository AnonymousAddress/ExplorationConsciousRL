...
class DDPG(object):
    def __init__(self, actor, critic, memory, observation_shape, action_shape, param_noise=None, action_noise=None,
        ...
        self.noises = tf.placeholder(tf.float32, shape=(None,) + action_shape, name='noises')
        self.prev_noises = tf.placeholder(tf.float32, shape=(None,) + action_shape, name='prev_noises')
        ...
        if self.alpha is not None and self.action_noise is not None and not self.Q1 and self.random_actor:
            ...
            for i in range(grad_num_samples):
                ...
                noise = tf.random_normal(tf.shape(self.actor_tf), mean=0.0, stddev=0.2)
                noisy_action = self.actor_tf + noise
                clipped_action = tf.clip_by_value(noisy_action, self.action_range[0], self.action_range[1])
                current_critic = critic(normalized_obs0, clipped_action, reuse=True)
                critic_with_actor_tf = denormalize(tf.clip_by_value(current_critic, self.return_range[0],self.return_range[1]), self.ret_rms)
                critic_with_actor_tf_list.append(critic_with_actor_tf)
            self.critic_with_actor_tf = tf.reduce_mean(tf.concat(critic_with_actor_tf_list, axis=1), axis=1, keepdims=True)
        else:
        ...
        if self.alpha is not None and self.action_noise is not None and not self.Q1:
            ...
            for i in range(alpha_num_samples):
                ...
                noise = tf.random_normal(tf.shape(action), mean=0.0, stddev=0.2)
                noisy_action = action + noise
                clipped_action = tf.clip_by_value(noisy_action, self.action_range[0], self.action_range[1])
                Q_obs1_list.append(denormalize(target_critic(normalized_obs1, clipped_action, reuse=reuse), self.ret_rms))
            Q_obs1 = tf.reduce_mean(tf.concat(Q_obs1_list, axis=1), axis=1, keepdims=True)
        else:
            action = tf.clip_by_value(action, self.action_range[0], self.action_range[1])
            Q_obs1 = denormalize(target_critic(normalized_obs1, action), self.ret_rms)
    ...
    def store_transition(self, obs0, action, reward, obs1, terminal1, noise, prev_noise):
        reward *= self.reward_scale
        self.memory.append(obs0, action, reward, obs1, terminal1, noise, prev_noise)
        if self.normalize_observations:
    ...
    def get_stats(self):
        ...
        values = self.sess.run(self.stats_ops, feed_dict={
            ...
            self.noises: self.stats_sample['noises'],
            self.prev_noises: self.stats_sample['prev_noises']
